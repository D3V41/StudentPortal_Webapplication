...
const SelectStudent = () => {
  ...
  const loadStudents = async () => {
    const result = await axios.get(
      `http://localhost:8080/studentreport/external/${localStorage.getItem(
        "id"
      )}`,
      config
    );
    ...
    for (var j = 0; j < students.length; j++) {
      input.forEach((element) => {
        if (element.studentId == students[j].studentId) {
          ...
          element["studentPhone"] = students[j].studentPhone;
          element["studentEmail"] = students[j].studentEmail;
        }
      });
    }
    return input;
